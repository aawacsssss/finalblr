...
const OngoingProjects: React.FC = () => {
  ...
  const loadProjects = async () => {
    try {
      setLoading(true);
      setError(null);
      const data = await projectService.getByStatus('devam');
      setProjects(data);
    } catch (err) {
                ...
                <div className="row">
                  ...
                  <div className="col-lg-4">
                    <Link to="/devam-eden-projeler">
                      <div className="ic-btn-2 text-center p-3">Devam Eden</div>
                    </Link>
                  </div>
                  <div className="col-lg-4">
        ...
        <div className="container">
          {projects.length > 0 ? (
            ...
            <div style={{ textAlign: 'center', padding: '60px 20px' }}>
              <h3 style={{ color: '#666', marginBottom: '16px' }}>Henüz Devam Eden Proje Yok</h3>
              <p style={{ color: '#888' }}>Şu anda devam eden projeler bulunmamaktadır.</p>
            </div>
                     )}
         </div>
       </section>
